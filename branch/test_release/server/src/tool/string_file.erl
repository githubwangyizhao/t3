...
%%-define(OUT, "../src/gen/sensitive.txt").

-define(OUT_KEY, "../src/gen/keycheck.erl").

-define(OUT_KEY_VAGUE, "../src/gen/keycheck_vague.erl").
...
%%    Map = sensitive:get(),
    Keys = maps:keys(Map),
    List = key_start(Keys, Map, [], 0),
    erase(key_value),
    file:write_file(?OUT_KEY, head_key() ++ List).

key_start([], _Map, List, _Count) ->        % 最尾部
    I = io_lib:format("mc (_, _) ->\n~sfs.\n ", [get_list_null(1)]),
    dict_format(List, 0) ++ [I];
key_start([Key | Keys], Map, List, Count) ->
    case maps:find(Key, Map) of
        {ok, Value} ->
            NewList = dict_format(List, 0),
            key_start(Keys, Map, key_map1(Value, Key, Count, 1, NewList), Count);
        true ->
            key_start(Keys, Map, List, Count)
    end.

key_map([], _Map, List, Count) ->       % 每次的结束
    ...
    List;
key_map([Key | Keys] = KeyList, Map, List, Count) when is_list(KeyList) ->
    case maps:find(Key, Map) of
        {ok, Value} ->
            key_map(Keys, Map, key_map1(Value, Key, Count, 1, List), Count);
        true ->
            key_map(Keys, Map, List, Count)
    end;
key_map(Key, Map, List, Count) ->
    case maps:find(Key, Map) of
        {ok, Value} ->
            key_map([], Map, key_map1(Value, Key, Count, 1, List), Count);
        true ->
            key_map([], Map, List, Count)
    end.

key_map1([], _, _, _, NewList) ->
    NewList;
key_map1([Map | Maps] = MapList, Key, Count, SingleCount, NewList) when is_list(MapList) ->
    F =
        fun(M) ->
            key_map1(M, Key, Count, SingleCount, [])
        end,

    if
        length(MapList)==2 andalso not is_list(Maps) ->
            key_map1([Maps], fs1, Count, SingleCount + 1, NewList ++ F(Map));
        true ->
            key_map1(Maps, fs1, Count, SingleCount + 1, NewList ++ F(Map))
    end;

key_map1(Map, Key, Count, SingleCount, List) ->
...
%%                            io:format("~p: _____~p_____~p~n", [?LINE, Map, Key]),
                ...
                true ->
                    ...
                    key_map(S, Map, NewList ++ [I], Count + 1)
            end;
        true ->
            if
                Map==fs ->
...
%%    Map = sensitive:get(),
    ...
    erase(key_value),
    file:write_file(?OUT_KEY_VAGUE, head_key_vague() ++ List).
