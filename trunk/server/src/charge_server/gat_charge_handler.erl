...
%%%%% @doc        港澳台
%%%%% Created : 21. 六月 2019 17:03
%%%%%-------------------------------------------------------------------
-module(gat_charge_handler).
%%-author("home").
%%
%%-export([
%%    init/2,
...
%%
%%%% @doc     港澳台平台充值
%%path_request(<<"/charge_gat">>, <<"GET">>, Ip, Req) ->
%%    {ParamList, ParamStr} = get_req_param_str(Req),
%%    ?INFO("港澳台平台充值:~p ~n", [ParamStr]),
%%    Money = util:str_to_float(get_list_value(<<"pay_money">>, ParamList)),      % 平台货币
%%    OrderId = util:to_list(get_list_value(<<"order_id">>, ParamList)),          % 订单号
%%    AccId = util:to_list(get_list_value(<<"user_id">>, ParamList)),             % 账号
%%    PlatformServerOrderIdStr = util:to_list(get_list_value(<<"params">>, ParamList)),   % 游戏参数
%%%%    mod_signature:check_param_list(ParamList, ?PLATFORM_GAT),
%%    platform_gat:check_param_list(ParamList),
%%    case string:tokens(PlatformServerOrderIdStr, "^") of
%%        [GameUrl, GameOrderNoStr] when is_list(GameUrl) ->
%%            jump_game_handler:jump_game_charge(?CHANNEL_QQ_GAME, GameUrl, OrderId, GameOrderNoStr, Money);
%%        _ ->
%%            mod_charge_server:server_charge(AccId, OrderId, Money, PlatformServerOrderIdStr, Ip, util_time:timestamp(), ?CHANNEL_QQ_GAME)
%%    end;
