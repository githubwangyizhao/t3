...
%% @doc 创建机器人
create_robot(RobotId, HeartBeatTime, DelayTimeMS, DestroyTimeMS) ->
    RobotConfigId = get_random_robot_config_id(get(?DICT_SCENE_ID)),
%%    RobotId = get_random_robot_id(),
    ObjSceneRobot = get_scene_robot(RobotConfigId, RobotId, DelayTimeMS, DestroyTimeMS),
    ...
    erlang:send_after(HeartBeatTime, self(), {?MSG_SCENE_ROBOT_HEART_BEAT, RobotId}).
create_robot(ConfigRobotId, RobotId) ->
    ...
    case ?GET_OBJ_SCENE_PLAYER(RobotId) of
        ?UNDEFINED when IsCanCreateRobot ->
            ObjSceneRobot = get_scene_robot(ConfigRobotId, RobotId),
...
get_scene_robot(ConfigRobotId, RobotId) ->
    get_scene_robot(ConfigRobotId, RobotId, 0, 0).
get_scene_robot(ConfigRobotId, RobotId, DelayTimeMS, DestroyTimeMS) ->
    #t_robot{
        hero_list = HeroList,
        vip_level = VipLevel,
    ...
    #db_c_game_server{
        sid = ServerId
    } = util_random:get_list_random_member(mod_server:get_game_server_list()),
    ...
    SceneMasterState =
        case MissionType of
            ?MISSION_TYPE_SHISHI_BOSS ->
                ?MISSION_TYPE_SHISHI_BOSS;
            _ ->
                {SceneMasterState1, _} = mod_scene_event_manager:get_state(),
                SceneMasterState1
        end,
    ShiShiIsFight = get(shi_shi_is_fight),
    if
        MissionType == ?MISSION_TYPE_SHISHI_BOSS andalso ShiShiIsFight == false ->
            Robot;
        TargetHp > 0 ->
                    ...
                    if TmpSKillInfo == null ->
                        ...
                        if
                            IsCommonSkill == false ->
                                ...
                                if
                                    SceneMasterState == ?MISSION_TYPE_SHISHI_BOSS andalso (SkillId == ?ACTIVE_SKILL_4 orelse SkillId == ?ACTIVE_SKILL_5) ->
                                        TmpSKillInfo;
