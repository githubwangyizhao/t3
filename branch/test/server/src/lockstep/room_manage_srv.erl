...
try_bind_room_if_exit(PlayerId, ClientWorker, SenderWorker) ->
	mod_server_rpc:gen_server_call_war(?SERVER, {try_bind_room_if_exit, PlayerId, ClientWorker, SenderWorker}).

leave_room(PlayerId) ->
	mod_server_rpc:gen_server_cast_war(?SERVER, {leave_room, PlayerId}).
	...
	Result =
		case get_ori_room_worker(PlayerId) of
			...
			{ok, _OriRoomWorker} ->
				...
				player_in_room_already
		end,
	{reply, Result, State};
handle_call({try_bind_room_if_exit, PlayerId, ClientWorker, SenderWorker}, _From, State) ->
	?DEBUG("try_bind_room_if_exit => PlayerId:~p", [PlayerId]),
	Result =
		case get_ori_room_worker(PlayerId) of
			null ->
				player_not_in_room;
...
handle_cast({leave_room, PlayerId}, State) ->
...
push_room_info_2_game_server(Type, RoomId) ->
	...
	mod_server_rpc:cast_all_game_server(mod_room, handle_receive_war_push, [Type, RoomId, PlayerBaseInfoList]).
