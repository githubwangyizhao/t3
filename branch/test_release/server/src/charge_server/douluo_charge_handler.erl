...
%%% @doc    斗罗修仙
%%% Created : 26. 十二月 2018 19:22
%%%-------------------------------------------------------------------
-module(douluo_charge_handler).
-author("home").
...
%%                {_, ParamStr} = get_req_param_str(Req),
    ...
    web_http_util:output_xml(Req, [{return_code, ResultText}, {return_msg, util_string:to_utf8(Msg)}]).
terminate(_Reason, _Req) ->
    ok.

%% @doc     斗罗修仙地址充值
path_request(<<"/game_charge_douluo">>, <<"POST">>, Ip, Req) ->
    {ParamList, ParamStr} = get_req_param_str(Req),
    ?INFO("斗罗修仙游戏服充值:~p ~n", [ParamStr]),
    OpenId = get_list_value(openid, ParamList),
    douluo:new_check_param_list(OpenId, ParamList),
    ReturnCode = get_list_value(return_code, ParamList),
    if
        ReturnCode == "SUCCESS" orelse ReturnCode == 'SUCCESS' ->
            ResultCode = get_list_value(result_code, ParamList),
            OrderId = get_list_value(out_trade_no, ParamList),
            if
                ResultCode == "SUCCESS" orelse ResultCode == 'SUCCESS' ->
                    Money = util:to_int(get_list_value(total_fee, ParamList)) / 100,
                    GameOrderNoStr = get_list_value(attach, ParamList),
                    mod_charge:web_game_charge(Ip, OrderId, GameOrderNoStr, Money, 10);
                true ->
                    ErrCodeMsg = get_list_value(err_code_des, ParamList),
                    ?ERROR("斗罗修仙>>ResultCode:~p OrderId:~p ~ts~n ", [ResultCode, OrderId, ErrCodeMsg]),
                    exit(null_object)
            end;
        true ->
            ErrReturnMsg = get_list_value(return_msg, ParamList),
            ?ERROR("斗罗修仙ReturnCode>>:~p ~ts~n ", [ReturnCode, ErrReturnMsg]),
            exit(null_object)
    end;
...
get_req_param_str(Req) ->
    charge_handler:get_req_param_xml(Req).
