...
build_table_map([TableFile | L], TableList, RecordOut, EnumOut, EnumConfigs, JsonConfigs) ->
    Parser = util_utf16le:read_file(TableFile),
...
%%    case csv:next_line(Parser) of
    case util_utf16le:read_line(Parser) of
        null ->
            lists:reverse(List);
        Data ->
            [Value|_] = Data,
            case Value of
                35 ->
                    get_all_csv_rows(Parser, List);
                _ ->
                    get_all_csv_rows(Parser, [lists:reverse(get_str_list(Data, "\t", [])) | List])
            end
    end.

get_str_list(OldStr, Str, NewList) ->
    get_str_list_1(OldStr, Str, NewList).
get_str_list_1(OldStr, Str, NewList) ->
    case string:split(OldStr, Str) of
        [Data] ->
            [Data | NewList];
        [Data1, Data2] ->
            get_str_list(Data2, Str, [Data1 | NewList])
    end.
...
%%                    {N + 1, ["" | TmpDataList]};
                ...
                ?TYPE_STR ->
                    ...
                    Data1 =
                        case lists:suffix(?LIST_FILED_SUF, Field) of
                            true ->
                                util_string:string_to_list_term(util_string:replace(util_string:replace(util_string:replace(Data0, "\"", ""), ")", "]"), "(", "["));
                            false ->
                                case lists:suffix(?TUPLE_FILED_SUF, Field) of
                                    true ->
                                        util_string:string_to_list_term(util_string:replace(util_string:replace(util_string:replace(Data0, "\"", ""), ")", "]"), "(", "["));
...
get_next_line(Parser) ->
    Data = util_utf16le:read_line(Parser),
    lists:reverse(get_str_list(Data, "\t", [])).
