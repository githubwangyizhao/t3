...
parse_binary_line(Rest, Str, Acc) ->
    parse_binary_line0(Rest, Str, Acc, true).

parse_binary_line0(<<$#, Rest/binary>>, Str, Acc, _) ->
    parse_binary_line(Rest, Str, Acc, false);
parse_binary_line0(<<$", $#, Rest/binary>>, Str, Acc, _) ->
    parse_binary_line(Rest, Str, Acc, false);
parse_binary_line0(Rest, Str, Acc, _) ->
    parse_binary_line(Rest, Str, Acc, true).


parse_binary_line(<<$\n, Rest/binary>>, Str, Acc, false) ->
    parse_binary_line(Rest, Str, Acc, true);
parse_binary_line(<<$\r, Rest/binary>>, Str, Acc, false) ->
    parse_binary_line(Rest, Str, Acc, true);
parse_binary_line(<<_, Rest/binary>>, Str, Acc, false) ->
    parse_binary_line(Rest, Str, Acc, false);
parse_binary_line(<<$\n, Rest/binary>>, Str, Acc, true) ->
   end_of_line(Rest, Str, Acc);
parse_binary_line(<<>>, Str, Acc, true) ->
    end_of_line(<<>>, Str, Acc);
parse_binary_line(<<$\r, Rest/binary>>, Str, Acc, true) -> %% NOTE: skip \r
    parse_binary_line(Rest, Str, Acc, true);
parse_binary_line(<<$", $,, Rest/binary>>, _Str, Acc, true) ->
    parse_binary_line(Rest, <<>>, ["\""|Acc], true);
parse_binary_line(<<$", Rest/binary>>, Str, Acc, true) ->
    parse_binary_string(Rest, Str, Acc);
parse_binary_line(<<$,, $\r, $\n,Rest/binary>>, Str, Acc, true) ->
    end_of_line(Rest, <<>>, [string:strip(binary_to_list(<<>>)), string:strip(binary_to_list(Str))|Acc]);
parse_binary_line(<<$,, $\n,Rest/binary>>, Str, Acc, true) ->
    end_of_line(Rest, <<>>, [string:strip(binary_to_list(<<>>)), string:strip(binary_to_list(Str))|Acc]);

parse_binary_line(<<$,, Rest/binary>>, Str, Acc, true) ->

    parse_binary_line(Rest, <<>>, [string:strip(binary_to_list(Str))|Acc], true);
parse_binary_line(<<I, Rest/binary>>, Str, Acc, true) ->
    parse_binary_line(Rest, <<Str/binary, I>>, Acc, true).

parse_binary_string(<<$", $,,$\r, $\n,Rest/binary>>, Str, Acc) ->
    end_of_line(Rest, <<>>, [string:strip(binary_to_list(<<>>)), string:strip(binary_to_list(Str))|Acc]);
parse_binary_string(<<$", $,, $\n,Rest/binary>>, Str, Acc) ->
    end_of_line(Rest, <<>>, [string:strip(binary_to_list(<<>>)), string:strip(binary_to_list(Str))|Acc]);
