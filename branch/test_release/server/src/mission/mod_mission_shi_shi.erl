...
handle_robot() ->
    ...
    if
        RobotNum < RobotNumLimit ->
            ...
            case mod_robot_player_scene_cache:get_robot_obj(SceneId) of
                null ->
                    noop;
                RobotObjSceneActor_0 ->
                    #obj_scene_actor{
                        robot_data = OldRobotData
                    } = RobotObjSceneActor_0,
                    RobotObjSceneActor = RobotObjSceneActor_0#obj_scene_actor{
                        key = {?OBJ_TYPE_PLAYER, RobotId},
                        obj_id = RobotId,
                        robot_data = OldRobotData#robot_data{
                            robot_delay_time_ms = util_time:milli_timestamp() + FightTime
                        }
                    },
                    mod_scene_robot_manager:do_create_robot(RobotObjSceneActor, DelayTime, FightTime + StayTime)
            end;
        true ->
            noop
    end,
