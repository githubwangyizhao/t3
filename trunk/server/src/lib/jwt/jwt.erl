...
%%    Claims = cow_base64url:encode(jsx:encode(ClaimsSet)),
%%    Header = cow_base64url:encode(jsx:encode(jwt_header(Alg))),
    Claims = base64:encode(jsx:encode(ClaimsSet)),
    Header = base64:encode(jsx:encode(jwt_header(Alg))),
...
decode_jwt(#{header := Header, claims := Claims} = Context) ->
    try
        [HeaderJSON, ClaimsJSON] =
            Decoded = [maps:from_list(jsx_decode_safe(base64:decode(X))) || X <- [Header, Claims]],
...
jwt_check_sig({rsa, Crypto}, Payload, Signature, Key) ->
    public_key:verify(Payload, Crypto, base64:decode(Signature), Key);
...
jwt_sign_with_crypto({hmac, Crypto}, Payload, Key) ->
    base64:encode(crypto:hmac(Crypto, Key, Payload));
...
jwt_sign_with_crypto({rsa, Crypto}, Payload, Key) ->
    base64:encode(public_key:sign(Payload, Crypto, Key));

jwt_sign_with_crypto({ecdsa, Crypto}, Payload, Key) ->
    base64:encode(jwt_ecdsa:signature(Payload, Crypto, Key));
