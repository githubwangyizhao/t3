...
%%
%%%% @fun 地址请求
%%path_request(<<"/game_charge_vivo">>, <<"POST">>, Ip, Req) ->     % vivo平台返回游戏服充值
%%    {ParamList, ParamStr} = get_req_param_str(Req),
%%    ?INFO("vivo平台返回游戏服充值:~s~n", [ParamStr]),
%%    mod_signature:check_param_list(lists:keydelete(<<"signMethod">>, 1, ParamList), ?CHANNEL_VIVO),
%%%%    PayStatus = util:to_int(get_list_value(<<"orderStatus">>, ParamList)),    % 充值状态
%%%%    if
%%%%        PayStatus == 0 ->
%%    OrderId = util:to_list(get_list_value(<<"cpOrderNumber">>, ParamList)),         % 平台订单号
%%    #db_charge_order_request_record{
%%        param_str = GameOrderNoStr
%%    } = mod_charge:get_charge_order_request_record_init(OrderId),
%%    ?ASSERT(GameOrderNoStr =/= "", {param_type_error, 'GameOrderNoStr==null'}),
%%%%    GameOrderNoStr = util:to_list(get_list_value(<<"orderDesc">>, ParamList)),      % 游戏服参数
%%    Money = util:str_to_float(get_list_value(<<"orderAmount">>, ParamList)),        % 人民币
%%    ?INFO("vivo平台返回成功:~p~n", [{GameOrderNoStr, OrderId}]),
%%    mod_charge:web_game_charge(Ip, OrderId, GameOrderNoStr, Money);
%%%%        true ->
%%%%            ?INFO("vivo平台返回无效:~p~n", [ParamStr]),
%%%%            exit(null_object)
%%%%    end;
