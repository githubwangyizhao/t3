...
-define(MYSQL_8_0, 80). %% Support for MySQL 8.0.x et 5.0.x

-define(PING_TIME, 60 * 60 * 1000).		%% 1 hour
...
loop(State) ->
	...
	ping ->
		erlang:send_after(?PING_TIME, self(), ping),
		case do_query(State, <<"select now();">>) of
			{error, Other} ->
				?Log2(LogFun, error,
					"ping error, exiting: ~p", [Other]),
				error;
			_Res ->
				loop(State)
		end;
...
%% Support for MySQL 8.0.x and 5.x:
get_fields(LogFun, RecvPid, Res, ?MYSQL_8_0) ->
    case do_recv(LogFun, RecvPid, undefined) of
	{ok, Packet, _Num} ->
	    case Packet of
		<<254:8>> ->
		...
		_ ->
		    ...
		    This = {Table,
			    ...
			    Length,
			    get_field_datatype(Type)},
		    get_fields(LogFun, RecvPid, [This | Res], ?MYSQL_8_0)
	    end;
	{error, Reason} ->
	    {error, Reason}
    end.
...
get_error_data(ErrPacket, ?MYSQL_8_0) ->
	<<Code:16/little, _M:8, SqlState:5/binary, Message/binary>> = ErrPacket,
	{Code, {binary_to_list(SqlState), binary_to_list(Message)}}.
