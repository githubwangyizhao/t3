...
%%%%% @doc        quick平台
%%%%% Created : 28. 五月 2019 20:09
%%%%%-------------------------------------------------------------------
-module(quick_charge_hankler).
%%-author("home").
%%
%%-export([
%%    init/2,
...
%%
%%%% @doc     quick地址充值
%%path_request(<<"/game_charge_quick">>, <<"POST">>, Ip, Req) ->
%%    {DataParamList, ParamStr} = get_req_param_str(Req),
%%    ?INFO("quick游戏服充值:~p ~n", [ParamStr]),
%%    DataStr = util:to_list(get_list_value(<<"nt_data">>, DataParamList)),
%%    Sign = util:to_list(get_list_value(<<"sign">>, DataParamList)),
%%    Md5Sign = util:to_list(get_list_value(<<"md5Sign">>, DataParamList)),
%%
%%    platform_quick:check_param_list(DataStr, Sign, Md5Sign),
%%
%%    ParamList = platform_quick:decode_data(DataStr),
%%    PayStatus = util:to_int(get_list_value(status, ParamList)),
%%    if
%%        PayStatus == 0 ->
%%            OrderId = get_list_value(game_order, ParamList),
%%            Money = util:str_to_float(get_list_value(amount, ParamList)),
%%            GameOrderNoStr = get_list_value(extras_params, ParamList),
%%            Channel = get_list_value(channel, ParamList),
%%            mod_charge:web_game_charge(Ip, OrderId, GameOrderNoStr, Money, Channel);
%%        true ->
%%            ?INFO("quick充值返回无效:~p~n", [ParamStr]),
%%            exit(null_object)
%%    end;
