...
%%%%% @doc        映客
%%%%% Created : 5. 十一月 2018 11:31
%%%%%-------------------------------------------------------------------
-module(yk_charge_handler).
%%-author("home").
%%
%%-export([
%%    init/2,
...
%%
%%%% @doc     映客平台充值
%%path_request(<<"/game_charge_yk">>, <<"POST">>, Ip, Req) ->
%%    {ParamList, ParamStr} = get_req_param_str(Req),
%%    ?INFO("映客平台充值:~p ~n", [ParamStr]),
%%    mod_signature:check_param_list(ParamList),
%%    PayStatus = util:to_int(get_list_value(<<"state">>, ParamList)),      % 充值状态
%%    if
%%        PayStatus == 1 ->
%%            Money = util:to_int(get_list_value(<<"total_fee">>, ParamList)) / 100,      % 人民币（分）
%%            OrderId = util:to_list(get_list_value(<<"tradeno">>, ParamList)),           % 订单号
%%            GameOrderNoStr = util:to_list(get_list_value(<<"attach">>, ParamList)),   % 游戏参数
%%            ?INFO("映客平台返回成功 ~p~n", [{GameOrderNoStr, OrderId}]),
%%            mod_charge:web_game_charge(Ip, OrderId, GameOrderNoStr, Money);
%%        true ->
%%            ?INFO("映客平台返回 无效 ~p~n", [ParamStr]),
%%            exit(null_object)
%%    end;
