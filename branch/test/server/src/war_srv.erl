...
%% ----------------------------------
get_join_node_list() ->
    mod_cache:get({?MODULE, join_node_list}, []).
...
update_join_node_list(List) ->
    mod_cache:update({?MODULE, join_node_list}, List).
...
%%    mod_server_rpc:gen_server_call_zone(?MODULE, join),
    mod_server_rpc:cast_all_game_server(mod_server, join_war, []).
