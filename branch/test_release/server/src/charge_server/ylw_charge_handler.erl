...
%%%%% @doc        易乐玩
%%%%% Created : 22. 十一月 2018 13:54
%%%%%-------------------------------------------------------------------
-module(ylw_charge_handler).
%%-author("home").
%%
%%-export([
%%    init/2,
...
%%                {error, result_msg(R1)}
%%        end,
%%    ErrorList = [{status, ErrorCode}, {info, Msg}, {'data', []}],
%%    Req2 = web_http_util:output_error_code(Req, ErrorList),
%%    Req2.
%%terminate(_Reason, _Req) ->
%%    ok.
%%%% @doc     易乐玩平台充值
%%path_request(<<"/charge_ylw">>, <<"GET">>, Ip, Req) ->
%%    {ParamList, ParamStr} = get_req_param_str(Req),
%%    UserId = util:to_list(get_list_value(<<"uid">>, ParamList)),                % 账号
%%    Money = util:str_to_float(get_list_value(<<"money">>, ParamList)),          % 人民币
%%    OrderId = util:to_list(get_list_value(<<"productOrderId">>, ParamList)),    % 订单号
%%    GameOrderNoStr = util:to_list(get_list_value(<<"comment">>, ParamList)),   % 游戏参数
%%    ?INFO("易乐玩平台充值:~p ~n", [ParamStr]),
