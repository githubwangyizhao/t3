...
%% ----------------------------------
%% @doc 	game服收到战区服推送的房间列表信息
%% @throws 	none
%% @end
%% ----------------------------------
handle_receive_war_push(Type, RoomId, PlayerBaseInfoList) ->
...
%% ----------------------------------
leave_room(PlayerId) ->
	room_manage_srv:leave_room(PlayerId),
	%% 取消订阅
	unSubscribe(PlayerId),
	ObjPlayer = mod_obj_player:get_obj_player(PlayerId),
	mod_obj_player:update_obj_player(ObjPlayer#ets_obj_player{room_type = 0, room_id = 0, room_worker = null}).
		...
		[R] ->
			...
			case util:is_pid_alive(RoomWorker) of
				true ->
					...
					case catch room_manage_srv:try_bind_room_if_exit(PlayerId, ClientWorker, SenderWorker) of
						ok ->
							true;
						_ ->
							false
