...
%%%%% @doc        魅族平台
%%%%% Created : 22. 五月 2019 15:06
%%%%%-------------------------------------------------------------------
-module(meizu_charge_handler).
%%-author("home").
%%
%%
%%-export([
...
%%    Code =
%%        if
%%            ErrorCode == 0 orelse ErrorCode == 411 ->
%%                200;
%%            true ->
%%                ErrorCode
%%        end,
%%    ErrorList = [{code, util:to_binary(util:to_list(Code))}],
%%    Req2 = web_http_util:output_error_code(Req, ErrorList),
%%    Req2.
...
%%
%%%% @fun 地址请求
%%path_request(<<"/charge_meizu">>, <<"POST">>, Ip, Req) ->     % 族平台返回充值服
%%    {ParamList, ParamStr} = get_req_param_str(Req),
%%    ?INFO("魅族平台返回充值服充值:~s~n", [ParamStr]),
%%    mod_signature:check_param_list(lists:keydelete(<<"sign_type">>, 1, ParamList), ?CHANNEL_MEIZU),
%%    OrderId = util:to_list(get_list_value(<<"partner_trade_no">>, ParamList)),  % 平台订单号
%%    Money = util:str_to_float(get_list_value(<<"total_fee">>, ParamList)),        % 人民币
%%    PlatformServerOrderIdStr = util:to_list(get_list_value(<<"desc">>, ParamList)),           % 游戏服参数
%%    case string:tokens(PlatformServerOrderIdStr, "^") of
%%        [GameUrl, GameOrderNoStr] when is_list(GameUrl) ->
%%            jump_game_handler:jump_game_charge(?CHANNEL_MEIZU, GameUrl, OrderId, GameOrderNoStr, Money);
%%        _ ->
%%            ?INFO("魅族平台返回成功:~p~n", [{PlatformServerOrderIdStr, OrderId}]),
%%            mod_charge_server:server_charge("", OrderId, Money, PlatformServerOrderIdStr, Ip, util_time:timestamp())
%%    end;
