...
log(Module, Line, error, FormatFun) ->
    ...
    io:format("[DB_ERROR]~w:~b: " ++ Format ++ "~n", [Module, Line] ++ Arguments);
...
start_link(PoolId, Host, undefined, User, Password, Database, LogFun,
    Encoding) ->
    start1(PoolId, Host, ?PORT, User, Password, Database, LogFun, Encoding, 1,
        start_link);
start_link(PoolId, Host, Port, User, Password, Database, LogFun, Encoding) ->
    start1(PoolId, Host, Port, User, Password, Database, LogFun, Encoding, 1,
        start_link).

start_link(PoolId, Host, Port, User, Password, Database, LogFun, Encoding, PoolSize) ->
    start1(PoolId, Host, Port, User, Password, Database, LogFun, Encoding, PoolSize,
        start_link).
...
start(PoolId, Host, Port, User, Password, Database, LogFun, Encoding, PoolSize) ->
    start1(PoolId, Host, Port, User, Password, Database, LogFun, Encoding, PoolSize,
        start).
...
start1(PoolId, Host, Port, User, Password, Database, LogFun, Encoding, PoolSize,
    ...
    gen_server:StartFunc(
        {local, ?SERVER}, ?MODULE,
        [PoolId, Host, Port, User, Password, Database, LogFun, Encoding, PoolSize], []).
...
init([PoolId, Host, Port, User, Password, Database, LogFun, Encoding, PoolSize]) ->
    LogFun1 = if LogFun == undefined -> fun log/4; true -> LogFun end,
    State = #state{log_fun = LogFun1},
    do_init(State, LogFun, PoolSize, [PoolId, Host, Port, User, Password, Database, LogFun, Encoding]).

do_init(State, _LogFun, 0, _) ->
    {ok, State};
do_init(State, LogFun, N, [PoolId, Host, Port, User, Password, Database, LogFun, Encoding]) ->
    case mysql_conn:start(Host, Port, User, Password, Database, LogFun, Encoding, PoolId) of
        {ok, ConnPid} ->
            Conn = new_conn(PoolId, ConnPid, true, Host, Port, User, Password, Database, Encoding),
            do_init(add_conn(Conn, State), LogFun, N - 1, [PoolId, Host, Port, User, Password, Database, LogFun, Encoding]);
        {error, Reason} ->
%%            ?Log2(LogFun, error,
%%                "MySQL connection failed:~p", [Reason]),
            {error, Reason}
    end.
